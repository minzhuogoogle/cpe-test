...
to the device's MQTT topic at a rate of one per minute.
Before you run the sample, you must follow the instructions in the README
for this sample at
https://github.com/GoogleCloudPlatform/python-docs-sample
s/tree/master/iot/api-client.
"""
...
def main():
    ...
    # Publish mesages to the MQTT bridge once per minute.
    data['project'] =  args.project_id
    data['registry'] = args.registry_id
    data['devicename'] = args.device_id
    ...
    while True:
        ...
        timestamp = datetime.datetime.now(pst_zone)
        data['timestamp'] = timestamp.replace(microsecond=0).isoformat(' ')
