...
#HEADER_FILE  = "gindex_header_template.html"
#FOOTER_FILE = "gindex_footer_template.html"
#INDEX_TABLE_FILE = "gindex_table_template.html"
#REGISTRY_TABLE_FILE = "registry.table.template.html"
#DEVICE_TABLE_FILE = "device.table.template.html"
#MESSAGE_TABLE_FILE = "message.table.template.html"
...
#query_test_detail_for_provision_test(bqclient, partner, testtype, testname, testdate, dst_file):
    print "enerate_daily_summary_report"
    indexquery="SELECT index, status, logfile, date FROM {}.{} \
                where partner=\"{}\" and type like \"%{}%\" and  date=\"{}\" order by index DESC".format(DATASET, TABLE, partner, testtype, testdate)
    print indexquery
    ...
    for _each in result:
       ...
       index = _each.index
       queryiotest = "SELECT index, name, cluster, version, disktype, loadbalance, \
               scale, region , project \
       ...
       for _itest in itestresult:
           ...
           append_lines_html_file(line, dst_file)
           line = "<td>{}</td>\n".format(_itest.disktype)
           append_lines_html_file(line, dst_file)
           line = "<td>{}</td>\n".format(_itest.loadbalance)
           append_lines_html_file(line, dst_file)
           line = "<td>{}</td>\n".format(_itest.scale)
           append_lines_html_file(line, dst_file)
           line = "<td>{}</td>\n".format(_itest.region)
           append_lines_html_file(line, dst_file)
           line = "<td>{}</td>\n".format(_itest.project)
           append_lines_html_file(line, dst_file)
...
order by status".format(DATASET, TABLE, sdate)
        ...
        if  passed  > 0 and  failed == 0  : 
           ...
           sstatus.append(0)
        daily_summary_file = "Elastifile.Provision_Test.{}.html".format(sdate)
    ...
    #hreflink = "a href='http://35.232.128.2/Elastifile.Provision_Test.{}.{}.html".format(testname, sdate)
    for i in sstatus:
        print i
        if i == 1:
        ...
        hreflink = "a href='http://35.232.128.2/Elastifile.Provision_Test.{}.html'".format(datelist[index])
        print hreflink
...
def query_test_detail_for_io_test(bqclient, partner, testtype, testname, testdate, dst_file):
    indexquery="SELECT index, status, logfile, date FROM {}.{} \
                where type=\"{}\" and name=\"{}\" and date=\"{}\" order by index DESC".format(DATASET,TABLE, testtype,  testname, testdate)
    result =  query_bigquerytable(bqclient, indexquery)
    #print indexquery
    copy_template_file_html_file(HEADER_FILE, dst_file)
...
def generate_summary_project_registry(bqclient, project, registry, dst_file):
    ...
    DEVICE_MSG_COUNT = "select count(index) as count, \
                    ...
                    group by  date, name  \
                    ORDER by  date DESC, name DESC".format(DATASET,TABLE, project, registry)
    ...
    for each_device in query_device_result:
        devicename = each_device.name.strip()
