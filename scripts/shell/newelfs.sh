...
# gcloud compute instances detach-disk   test-pssd-elfs-elfs-08e6beef	 --disk=test-pssd-elfs-elfs-08e6beef-external-ssd-01
    echo "detaching disk" 
    enode=$1
    export diskindex=`gcloud compute instances describe  $enode --project=$project  --zones=$zone --format="text(disks[].index)"   |  tail -n 1 |  cut -d ' ' -f2`
    diskindex=$((diskindex-1))
    export diskname=`gcloud compute instances describe $enode  --project=$project  --zones=$zone  --format="text(disks[$diskindex].deviceName)" | grep $disktype-elfs-elfs | cut -d ' ' -f2 `
    echo $diskname
...
#https://raw.githubusercontent.com/minzhuogoogle/cpe-test/master/fio/elastifile/fio.data.verify
    ...
    failure_node=`gcloud compute instances list --project $project --filter=enode_name | grep -v NAME | cut -d ' ' -f1 | tail -n 1`
    traffic_node=`gcloud compute instances list --project $project --filter=enode_name | grep -v NAME | cut -d ' ' -f1 | head -n 1`
    echo $failure_node $traffic_node
    delaytime=2
    export now=`date +"%s"`
