...
prepare_io_test () {
    ...
    echo "cmd: gcloud compute instances list --project=$project --filter=$enodename  --format=\"value(networkInterfaces[0].networkIP)\" | head -n 1"
    echo "cmd: gcloud compute instances list --project=$project --filter=$enodename  --format=\"value(networkInterfaces[0].networkIP)\""
...
run_test() {
    ...
    do
	    ...
	    echo "enode: $nfs_server  time: $timer, $newtimer  , $testname"
